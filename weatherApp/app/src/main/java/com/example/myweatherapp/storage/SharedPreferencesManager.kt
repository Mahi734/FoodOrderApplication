package com.example.myweatherapp.storage

import android.content.Context
import androidx.core.content.edit
import com.example.myweatherapp.data.CurrentLocation
import com.google.gson.Gson
import kotlinx.coroutines.currentCoroutineContext

//abstact class implementation provide by android system application level operation and toJson and fromGson

class SharedPreferencesManager(context: Context, private val gson:Gson) {

    private companion object{
        const val PREF_NAME = "WeatherAppPref"
        const val KEY_CURRENT_LOCATION = "currentLocation"
    }
//retrive and hold the contents return a shared Preferences you can retrive and modify
    //MODE_PRIVATE = file creation the created file only access by the application owned
    private val sharedPreferences = context.getSharedPreferences(PREF_NAME,Context.MODE_PRIVATE)


    fun saveCurrentLocation(currentLocation: CurrentLocation)
    {
        val currentLocationJson = gson.toJson(currentLocation)
        sharedPreferences.edit{
          putString(KEY_CURRENT_LOCATION,currentLocationJson)
        }
    }
    fun getCurrentLocation():CurrentLocation?{
        return sharedPreferences.getString(
            KEY_CURRENT_LOCATION,null
        )?.let{
            currentLocationJson ->
            gson.fromJson(currentLocationJson,CurrentLocation::class.java)
        }
    }
}